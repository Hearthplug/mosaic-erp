...
6. **Early sentinels.** Fresh sentinel families are held out from training in a separate deterministic `v6_sentinel.jsonl`; the authoritative dataset contains exactly train and development. The authoritative train split must mathematically yield at least 60 optimizer steps under the frozen batch size 2, gradient accumulation 16, and 3 epochs. Checkpoints 40 and 60 must score schema validity 100%, unknown labels 0, and minimum recall 90% on the four boundary groups. A miss stops the run before full training. Gate timing is derived from the measured experiment-6 learning curve on the frozen trainer: at step 20 (0.87 epochs) eval token accuracy was still 0.90 mid-descent, so format lock-in cannot be required there; step 40 is ~1.74 epochs. Sentinels are not the final development set and cannot be used for checkpoint selection beyond stop/go.
