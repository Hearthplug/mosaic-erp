#!/usr/bin/env python3
"""Fail-fast step-20/60 sentinel gate; never substitutes for final development."""
import argparse,json,pathlib,sys
V3=pathlib.Path(__file__).parents[1]/'v3';sys.path.insert(0,str(V3))
from runtime import load_contract,prediction_errors
C=load_contract(V3)
def main():
 p=argparse.ArgumentParser();p.add_argument('--step',type=int,choices=(20,60),required=True);p.add_argument('--dataset',required=True);p.add_argument('--predictions',required=True);a=p.parse_args()
