...
class AllPacks(unittest.TestCase):
 def test_all_22_are_versioned_sourced_and_fail_closed(self):
  ...
  for country,p in x.items():
   self.assertEqual(p['verification_state'],'requires_local_professional',country);self.assertEqual(p['statutory_adapter_state'],'disabled',country);self.assertTrue(p['sources'],country);self.assertTrue(p['pack_hash'],country);self.assertEqual(p['schema_version'],1)
