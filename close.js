...
async function loadCloses(){
 ...
 try{
  ...
  d.closes.forEach(c=>{
   ...
   tr.innerHTML='<td><b>'+esc(c.date)+'</b></td><td data-label="Counted cash" class="num">'+money(c.counted_cash_minor)+'</td><td data-label="Difference" class="num">'+diff+'</td><td data-label="Note">'+esc(c.note||'—')+'</td><td data-label="Status">'+chip+'</td>';
   tb.appendChild(tr);
  });
...
async function save(){
 ...
 try{
  ...
  $('#savehint').textContent='Saved. The day stays open - late edits simply mark this close as changed.';
  $('#note').value='';
  await loadCloses();
