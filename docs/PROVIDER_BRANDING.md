...
The sign-in screen packages the providers' published assets locally so its strict Content Security Policy does not depend on third-party images.
...
Recheck both provider pages before a later branding update because providers can change their required patterns. Mosaic's current source copies have these SHA-256 values:
