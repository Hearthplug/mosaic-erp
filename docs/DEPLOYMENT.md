...
The local assistant is an optional Preview feature since v1.3.0 and is bundled with the desktop apps; on server deployments it is not part of the core image and stays gated behind its measured availability check. Native benchmark evidence jobs pass when they complete valid, checksummed verdict artifacts; a `local-assistant-verdict` job validates both checksummed artifacts and records available `true` or `false`. The separate `local-assistant-availability` promotion gate runs and passes only when that verdict is `true`; it is skipped for a valid rejection, so rejected research does not turn core release checks red. The UI and runtime stay unavailable on missing or rejected verdicts. Availability requires that its pinned model passes measured CPU-only amd64/arm64 resource and typed-intent quality gates. No model or container download starts until chat shows the exact source, license, bytes, verified hashes and measured requirements, and the owner confirms the network and disk change.
