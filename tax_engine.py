...
class TaxEngine:
 ...
 def calculate(self,wid,transaction_date,jurisdiction,code,gross_or_net_minor,scope='domestic',business_customer=False):
  ...
  if not r:raise Conflict('No owner- or professionally-reviewed tax rule covers this date, place and transaction')
 ...
 def attest(self,wid,actor,data,rules):
  required=('jurisdiction','rules_version','reviewer_kind','reviewer_name','verified_on','effective_from','registration_scope','supply_scope','item_classification_basis','price_basis','rounding_basis','source_urls','limitations')
  ...
  if data['reviewer_kind'] not in ('owner','professional'):raise ValueError('reviewer_kind must be owner or professional')
  ...
  with self.s.tx():
   ...
   self.s._db.execute("UPDATE generated_artifacts SET status='reverification_required',legal_status='review_required' WHERE workspace_id=? AND kind='statutory_invoice' AND status='active' AND COALESCE(rules_version,'')<>?",(wid,data['rules_version']))
   self.s._db.execute('INSERT INTO tax_verifications(id,workspace_id,jurisdiction,rules_version,reviewer_kind,reviewer_name,credentials,verified_on,effective_from,effective_to,registration_scope,supply_scope,item_classification_basis,price_basis,rounding_basis,source_urls_json,limitations,attestation_hash,status,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,\'active\',?,?)',(vid,wid,data['jurisdiction'],data['rules_version'],data['reviewer_kind'],data['reviewer_name'],data.get('credentials',''),data['verified_on'],data['effective_from'],data.get('effective_to'),data['registration_scope'],data['supply_scope'],data['item_classification_basis'],data['price_basis'],data['rounding_basis'],canon(data['source_urls']),data['limitations'],h,actor,utcnow()))
   ids=[]
   for r in rules:
    rid='txr_'+secrets.token_hex(8);rule_payload={'j':data['jurisdiction'],'c':r['code'],'v':data['rules_version'],'from':data['effective_from'],'to':data.get('effective_to'),'rate':str(r['rate_percent']),'inclusive':data['price_basis']=='inclusive','scope':r.get('scope','domestic')};self.s._db.execute('INSERT INTO tax_rules(id,workspace_id,jurisdiction,code,version,effective_from,effective_to,rate_bps,price_includes_tax,scope,status,verified_by,verified_at,source_url,rules_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(rid,wid,data['jurisdiction'],r['code'],data['rules_version'],data['effective_from'],data.get('effective_to'),int(Decimal(str(r['rate_percent']))*100),int(data['price_basis']=='inclusive'),r.get('scope','domestic'),'verified',data['reviewer_name'],utcnow(),r['source_url'],sha256(canon(rule_payload))));ids.append(rid)
   self.s._audit(wid,actor,'tax.verification.attest',{'id':vid,'jurisdiction':data['jurisdiction'],'rules_version':data['rules_version'],'rules':ids,'attestation_hash':h,'statutory_adapter_state':'disabled','reviewer_kind':data['reviewer_kind']})
  return {'id':vid,'status':'active','attestation_hash':h,'verified_rules':ids,'statutory_adapter_state':'disabled','reviewer_kind':data['reviewer_kind'],'professional_review_recommended':data['reviewer_kind']=='owner'}
