...
class Verification(unittest.TestCase):
 ...
 def data(self,price='exclusive'):return {'jurisdiction':'Singapore','rules_version':'review-1','professional':'Local Tax LLP','credentials':'Registered tax adviser','verified_on':'2026-09-18','effective_from':'2026-01-01','registration_scope':'GST registered domestic retailer','supply_scope':'Domestic B2C goods only','item_classification_basis':'Reviewed SKU mapping v1','price_basis':price,'rounding_basis':'half-up per line','source_urls':['https://www.iras.gov.sg/'],'limitations':'No exports, reverse charge, filing or e-invoicing'}
 ...
 def test_attestation_enables_only_reviewed_math_not_statutory_output(self):
  a=self.t.attest(self.w,'owner',self.data(),[{'code':'STD','rate_percent':9,'source_url':'https://www.iras.gov.sg/'}]);self.assertEqual(a['statutory_adapter_state'],'disabled')
 ...
 def test_every_candidate_produces_explicit_professional_checklist(self):
