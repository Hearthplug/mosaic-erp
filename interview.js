...
function moduleReviewHtml(i){if(!i.module_review||!i.module_review.length)return'';let on=i.module_review.filter(m=>m.enabled),off=i.module_review.filter(m=>!m.enabled);let li=m=>'<li><b>'+m.label+'</b><span>'+m.plain+'</span></li>';return'<div class="mod-groups"><div class="mod-col"><p class="mod-head on">On for you</p><ul>'+on.map(li).join('')+'</ul></div><div class="mod-col off"><p class="mod-head off">Left off for now</p><ul>'+(off.length?off.map(li).join(''):'<li class="mod-none">Everything Mosaic offers is on.</li>')+'</ul></div></div><p class="mod-note">Left-off modules stay hidden so daily work stays simple. If your business changes, tell Mosaic in plain words below - the review updates before anything is saved.</p>'}
function showSummary(){loadJev();$('#interview').hidden=true;$('#summary').hidden=false;rail(session.status==='applied'?3:2);let i=session.inference;$('#review').innerHTML='<b>Business:</b> '+(i.owner_summary.business||'Your business')+'<br><b>First goal:</b> '+(i.owner_summary.first_goal||'Not answered')+'<br><b>Why features are enabled:</b><ul>'+i.explanations.map(x=>'<li>'+x.because+'</li>').join('')+'</ul>'+moduleReviewHtml(i)+(session.contradictions.length?'<b>One thing to clarify:</b> '+session.contradictions.map(x=>x.message).join(' '):'<b>Ready for owner and professional review.</b>');if(session.status==='applied')showApplied()}
